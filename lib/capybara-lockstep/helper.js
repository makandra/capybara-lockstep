window.CapybaraLockstep = (function() {
  ...
  function trackTurbo() {
    // Turbo defers its fetch() call behind an `await` in FetchRequest.perform(),
    // which means the actual fetch happens in a microtask after the click handler
    // returns. Our fetch wrapper only tracks work when fetch() is called, so there's
    // a gap between the click event and the fetch that can cause lockstep to report
    // idle prematurely.
    //
    // turbo:before-fetch-request fires synchronously inside that same microtask,
    // just before fetch(). We call startWork + stopWork here to keep lockstep busy
    // just long enough (via afterWaitTasks) for the actual fetch() to fire and be
    // picked up by trackFetch(). We don't track the full Turbo request lifecycle
    // because aborted requests (AbortError) don't fire turbo:fetch-request-error,
    // which would leave lockstep permanently busy.
    document.addEventListener('turbo:before-fetch-request', function() {
      startWork('turbo:fetch')
      stopWork('turbo:fetch')
    })
  }
  ...
  function synchronize(callback) {
    if (isIdle()) {
      // Wait one animation frame + one task to let layout-dependent
      // callbacks (like IntersectionObserver) fire and register new work.
